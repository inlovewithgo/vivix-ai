...
// 			<Sidebar />
// 			<main className="flex-1 bg-[#faf8ff] overflow-y-auto" ref={scrollDivRef}>
// 				<div className="p-4">
...
// 							<SuggestionCard
// 								icon={<Calculator />}
// 								text="Create a feature-rich calculator with basic arithmetic, scientific functions, and history tracking"
// 							/>
// 						</div>}
...
// 						<div className={`mt-8 p-6 bg-white rounded-xl border-2 border-[#FF1B8D]/20 fixed bottom-4 ${isOpen ? "w-[calc(100%-18rem)]" : "w-[calc(100%-7rem)]"} mx-auto transition-all duration-300`}>
...
import { SuggestionCard } from "@/components/suggestion-card"
import { Button } from "@/components/ui/button"
import { Gamepad2, Calculator, Cloud, ChevronLeft, Send, Image, BarChart3, PlusCircle, Trash } from 'lucide-react'
import { useGlobal } from "@/context/GlobalStateContext"
import { useEffect, useRef, useState } from "react";
import axios from "axios";
import { toast } from "react-toastify";
	...
	return (
		<div className="flex max-h-screen overflow-hidden">
			...
			<div className={`w-64 h-screen bg-white border-r flex flex-col transition-all duration-300 ease-in-out transform overflow-hidden`}>
				...
				}} variant="default" color="primary" className="mx-4 bg-gradient-to-r from-[#ffdbad] to-[#ff336f]">
					New Chat
				</Button>
						...
						<ContextMenu key={index}>
							...
							<ContextMenuContent className="w-64">
								<ContextMenuItem onClick={() => {
									...
									const username = storage?.getItem('username');
									if (!username) return toast.error("Please sign in to continue");
									const agentD = agents.find((item) => item.id == agent.id);
									if (!agentD) return toast.error("Agent not found");
									const clearedAgent = agents.filter((item) => item.id != agent.id);
									setHistory([]);
									setSelectedAgent("");
									setAgents(clearedAgent);
									localStorage.setItem("agents", JSON.stringify(clearedAgent));
									toast.success("Chat cleared successfully");
								}} inset className="text-red-500 font-semibold cursor-pointer">
									Delete
									<ContextMenuShortcut><Trash size={"17"} /></ContextMenuShortcut>
								</ContextMenuItem>
							</ContextMenuContent>
						</ContextMenu>
					))}
			...
			<main className="flex-1 overflow-y-auto mb-[10.5rem]" ref={scrollDivRef}>
				{!selectedAgent ? <div className="p-4 h-full">
					...
					<div className="flex items-center gap-4 mb-6">
						...
						<div className="flex items-center gap-2">
							<Image className="w-5 h-5 text-[#FF1B8D]" />
							<h1 className="text-xl font-semibold text-black">Agent Chat</h1>
						</div>
					</div>
					...
					<div className="max-w-full mx-auto text-center mb-12 relative">
						...
						<h2 className="text-3xl font-bold mb-4 text-black">Vivix AI</h2>
						<p className="text-gray-600 mb-8">
							Describe your idea here, and we'll bring it to life!
						</p>
						...
						{history.length !== 0 && <div className="">
							{history.map((item, index) => (
								<div key={index} className={`flex items-center gap-3 ${item.role != "user" ? "justify-start" : "justify-end"} gap-4 mb-4`}>
									{item.role != "user" && <div className={`w-10 h-10 rounded-lg flex-shrink-0`}>
										<img src="/logo-v.png" alt="logo" className="w-full h-full" />
